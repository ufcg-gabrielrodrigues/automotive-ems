...
if (alternatorCalcParamFlag)
    ...
    alternator.m_f.function = extFunctionHandle(@(i_f, n_r) ...
        (openCircuitVoltage(i_f, n_r)./(i_f.*n_r)).* ...
        ((sqrt(2).*30)./(p.*pi)));                                              % Indutância mútua entre fase do circuito de armadura e circuito de excitação [V/((rad/s)*A)]
